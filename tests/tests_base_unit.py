...
class TestBaseUnit(unittest.TestCase):
    def setUp(self):
        ...
        type_obj = Type(self.id_, self.alias, self.super, self.prefix, self.suffix, self.separator, self.function)
        self.base_unit = BaseUnit("test", type_obj)

    def test_base(self):
        # Test setting and getting the base property
        new_type_obj = Type(Id("new_id"), Alias("new_alias"), Super("new_super"), Prefix("new_prefix"), Suffix("new_suffix"), Separator("new_separator"), Function("new_function"))
